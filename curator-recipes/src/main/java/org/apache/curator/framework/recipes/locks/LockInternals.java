...
public class LockInternals {
    ...
    String attemptLock(long time, TimeUnit unit, byte[] lockNodeBytes) throws Exception {
        ...
        while (!isDone) {
            ...
            try {
                ...
                // 判断创建的临时节点是否是根节点下最小的节点，如果是返回true，加锁成功。
                hasTheLock = internalLockLoop(startMillis, millisToWait, ourPath);
            } catch (KeeperException.NoNodeException e) {
                // 当无法找到锁节点时，StandardLockInternalsDriver会抛出该错误，这可能在会话过期时发生。等等。因此，如果重试允许，请再次尝试所有操作
     ...
     * @param millisToWait      客户端获取锁的最长等待时间
     * @param ourPath           创建临时节点路径
     * @return
     * @throws Exception
     */
    private boolean internalLockLoop(long startMillis, Long millisToWait, String ourPath) throws Exception {
        ...
        try {
            ...
            // 进入循环，获取根节点下面所有子节点，判断上一步中创建的临时节点是否是根节点下最小的节点。如果是直接返回。
            while ((client.getState() == CuratorFrameworkState.STARTED) && !haveTheLock) {
                ...
                if (predicateResults.getsTheLock()) {
                    ...
                    synchronized (this) {
                        try {
                            // 对当前节点前一个节点加watch：使用getData()而不是exist()以避免留下不必要的观察者，这是一种资源泄漏
